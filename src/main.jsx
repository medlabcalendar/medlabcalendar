...
function FeatureStyles() {
  return (
    <style>{`
      ...
      .urgent-event { border: 2px solid #fde047; box-shadow: 0 12px 30px rgba(113, 63, 18, 0.08); }
...
function App() {
  ...
  const filteredEvents = sourceEvents.filter((event) => {
    ...
    const matchesMonth = showArchive || (
      event.startDate.getMonth() === currentCalendarMonth.getMonth() &&
      event.startDate.getFullYear() === currentCalendarMonth.getFullYear()
    )

    return matchesSearch && matchesCategory && matchesFormat && matchesFree && matchesMonth
  })
  ...
  return (
    <div className="page">
      ...
      <main>
                  ...
                  <div className="event-top">
                    <div>
                      <p className="event-title">
                        {event.isUrgente && <span style={{ color: '#a16207', fontWeight: 'bold', fontSize: '0.75rem', marginRight: '0.4rem' }}>⚠️ LIMITE INSCRIÇÃO:</span>}
                        {event.title}
                      </p>
                      <p className="muted">{event.organizer}</p>
                    </div>
        ...
        <MonthlyCalendar events={activeEvents} onMonthChange={(month) => setCurrentCalendarMonth(month)} />
          ...
          <div className="container">
            <div className="section-head">
              <div>
                ...
                <h2>
                  {showArchive 
                    ? 'Arquivo Histórico de Formações' 
                    : `Formações em ${monthNames[currentCalendarMonth.getMonth()]} ${currentCalendarMonth.getFullYear()}`}
                </h2>
              </div>
            ...
            <div className="notice">
              {showArchive 
                ? 'Arquivo de eventos passados, preservado para consulta histórica.' 
                : `A apresentar os resultados ativos localizados no mês corrente do calendário superior.`} 
