...
function MonthlyCalendar({ events }) {
  ...
  const firstUpcoming = events.find((event) => !event.isArchived)?.startDate || today
  const [visibleMonth, setVisibleMonth] = useState(new Date(firstUpcoming.getFullYear(), firstUpcoming.getMonth(), 1))
            ...
            {categories.map((category) => {
              ...
              return (
                <div className="category-panel" key={category}>
                  ...
                  ) : <p className="muted">Sem eventos ativos nesta categoria por enquanto.</p>}
                </div>
              )
            })}
          </div>
